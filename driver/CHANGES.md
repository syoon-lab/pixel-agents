# 변경 기록 (PLAN_ref.md 1단계 구현)

> 슬라이드 6번 "수정 & 변경 기록" 양식: [변경] 무엇을 / 이유: 왜.
> 서버(`server/`)·웹뷰(`webview-ui/`)·코어(`core/`)는 **한 줄도 수정하지 않음** (PLAN_ref.md 1단계 원칙).
...
- **[변경]** "한 번 실행당 5회" = 에이전트당 5단계로 해석 / **이유:** PLAN_ref.md의 무한 루프와 슬라이드 5회 제약을
