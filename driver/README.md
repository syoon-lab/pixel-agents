...
**가장 흔한 원인 — 실행 순서.** 반드시 **서버 → 브라우저 열기 → 드라이버** 순서로 실행한다.
pixel-agents 웹뷰는 접속 시점에 이미 존재하는 캐릭터(`existingAgents`)를 `layoutLoaded`
이후 버퍼에서 그리는데, 서버가 두 메시지를 역순(`layoutLoaded` 먼저)으로 보내는 기존 버그가
있어 **캐릭터가 생긴 뒤에 브라우저를 열거나 새로고침하면 빈 오피스로 보인다.** 브라우저를
**먼저 열어두면** 실시간 `agentCreated` 경로로 정상 표시된다. (캐릭터가 안 보이면: 브라우저를
켜둔 채 드라이버를 다시 실행 — 새로고침 말고.)

그 다음 원인 — 채택 디렉터리. 캐릭터는 서버가 스캔하는 `~/.claude/projects/<워크스페이스-해시>/`
에 트랜스크립트가 생길 때 채택된다. 드라이버의 워크스페이스 = **서버를 띄운 폴더**와 같아야 한다.
